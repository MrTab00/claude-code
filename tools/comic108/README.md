...
開くとまず地図が出る。**公式配置図のホール構成に沿った島レイアウト**で、
配置が取れたスペースだけが色付きで乗る。押すとそのサークルの詳細パネルが開く。

- **拡大縮小**: ＋ / − / 全体。縮小すると文字を消して全体の当たりだけ見せ、
  拡大するとスペース番号とサークル名を出す（読めない字を並べても意味がないため）
- 島は配置図と同じ 2 列の蛇行（右列を下から 1..N/2、左列を上から N/2+1..N）
- ブロックの並びも配置図と同じ向き（ア / あ / a が右端）
- ★を付けたサークルは地図上でも塗り分く

一覧が要るときは **カード / 表** に切り替える。

構成は `src/render/venue.ts` に持っていて、公式配置図 (C108Map_all_B4.pdf) から起こしたもの:
構成は `src/render/venue.ts` に持っていて、公式配置図 (C108Map_all_B4.pdf) から起こしたもの:
...
検索・日程・地区・状態の絞り込みは地図にもそのまま効くので、
「この作品のサークルは東のどこに固まっているか」が地図上で見える。

ブロック記号が分かればホールは一意に決まるので、**ツイートにホール番号が無くても正しい箱に入る**。
表記の揺れ（南を大文字で書く等）や、書き間違えたホール番号もブロック記号側に合わせて直す。
構成表に無いブロックは捨てずに「要確認」枠へ出す（解析ミスに気づけるように）。

**通路や島の細かな位置までは再現していない。** 現地で歩く順路は
