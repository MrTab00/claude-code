...
サークルタブの上に、採集できた配置を **日程 → 地区 → ホール → ブロック** で並べた格子が出る。
マスの数字はそのブロックにいるサークル数で、クリックすると一覧がそのブロックだけに絞られる。

**公式の会場平面図ではない。** ホール内の物理的な並びは採集データからは分からないので、
それらしい図を描くと現地で迷わせる。ここに出るのは実データ由来の索引だけ。
正確な配置図は [コミケWebカタログ](https://webcatalog.circle.ms/) を見ること。

検索や日程・地区の絞り込みを掛けると格子もそれに追随するので、
「鳴潮の本を出しているサークルは東のどのブロックに固まっているか」のような見方ができる。
