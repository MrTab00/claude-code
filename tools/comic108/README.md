...
サークルタブの上に、**公式配置図のホール構成に沿った**ブロック格子が出る。
...
マスの数字はそのブロックのサークル数。クリックで一覧がそのブロックだけに絞られ、
検索・日程・地区の絞り込みにも追随するので「この作品のサークルは東のどこに固まっているか」が見える。
...
**会場平面図ではない。** ホール内の通路や島の物理的な並びは持っていないので、
現地で歩く順路は [コミケWebカタログ](https://webcatalog.circle.ms/) の正確な配置図で確認すること。
