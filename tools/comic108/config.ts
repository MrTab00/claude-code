...
export const collect = {
     ...
     * 搜索关键词。f=live 取最新时间线。
     *
     * ハッシュタグ記号は付けない。X は本文の語もハッシュタグも同じ語として索引するので、
     * 「C108 お品書き」は「#C108 #お品書き」の投稿も本文に書いただけの投稿も両方拾う。
     * 「#C108」と書くとハッシュタグを使った投稿しか当たらず、その時点で取りこぼす。
     * おまけに # はシェルでコメント開始になるので、引数で渡すときの事故もなくなる。
     *
     * コスプレ系は「#C108コスプレ」のような連結ハッシュタグを当てにいくと外れる —— 実際に
     * 空振りした。語句の AND 検索の方が確実に当たる。
    ...
    queries: [
        ...
        'C108 スペース',
        'C108 配置',
        // 「頒布します」「サークル参加します」は告知の定型句で、上の 4 語では当たらない
        'C108 頒布',
        'C108 サークル参加',
        'コミケ108 お品書き',
        'C108 コスプレ',
     ...
     * これ以上割れない窓(1 日)で採る上限。
     *
     * 1 日まで割った窓が上限に当たっても、もう割りようがない。そこで打ち切ると
     * いちばん投稿の多い日 —— 開催直前 —— を採り切らずに終わってしまう。
     * 割れない窓だけは深く掘る。時間はかかるが、そこが本命なので払う価値がある。
     */
    maxTweetsWhenUnsplittable: 1800,

    /**
     * 検索を始める前に残しておきたい呼び出し回数。
     *
     * 深い検索は 30 回近く使う。残りがこれを下回ったまま始めると、途中でレート制限に
     * 当たって最初から引き直しになり、同じ期間に 2 倍の回数を使ってしまう。
     * 足りないときは先に回復を待つ。
     */
    minBudgetToStart: 32,
